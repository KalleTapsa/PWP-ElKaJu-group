...
from . import db
from .models import Image, Place, ReportImage, ReportPlace, ReportReview, Review, User
...
def get_all_places(trust_score=0.0, longitude=None, latitude=None, radius=None):
...
def get_places_by_category(
    category, trust_score=0.0, longitude=None, latitude=None, radius=None
):
    """Get places by category with optional filtering"""
    query = Place.query.filter(
        Place.category == category, Place.trust_score >= trust_score
    )
    if longitude is not None and latitude is not None and radius is not None:
        query = query.filter(
            (Place.longitude >= longitude - radius)
...
def get_places_by_application(
    application, trust_score=0.0, longitude=None, latitude=None, radius=None
):
    """Get places by application with optional filtering"""
    query = Place.query.filter(
        Place.application == application, Place.trust_score >= trust_score
    )
    if longitude is not None and latitude is not None and radius is not None:
        query = query.filter(
            (Place.longitude >= longitude - radius)
...
def create_review(user_id, place_id, rating, text=None):
    ...
    review = Review(
        user_id=user_id, place_id=place_id, rating=rating, text=text, trust_score=4.0
    )
...
def get_images_by_place(place_id, trust_score=0):
    ...
    return Image.query.filter(
        Image.place_id == place_id, Image.trust_score >= trust_score
    ).all()
...
def create_report_place(user_id, place_id, report_type: ReportType):
    ...
    if report:
        ...
        report = ReportPlace(
            user_id=user_id, place_id=place_id, report_type=report_value
        )
...
def create_report_review(user_id, review_id, report_type: ReportType):
    ...
    if report:
        ...
        report = ReportReview(
            user_id=user_id, review_id=review_id, report_type=report_value
        )
        db.session.add(report)
...
def create_report_image(user_id, image_id, report_type: ReportType):
    ...
    if report:
        ...
        report = ReportImage(
            user_id=user_id, image_id=image_id, report_type=report_value
        )
        db.session.add(report)
...
def recalculate_place_trust_score(place_id):
    ...
    weights = [REPORT_WEIGHTS[ReportType(report.report_type)] for report in reports]
